...
struct Task
{
    struct promise_type
    {
        ...
        Task get_return_object() { 
            ...
            // gThreadPool->addTask([coro = std::move(coro)] -> void { coro.resume(); })
            return Task(coro);
        }
...
struct Task<void>
{
    struct promise_type
    {
        ...
        auto initial_suspend() { return std::suspend_always{}; }
        auto final_suspend() noexcept { return std::suspend_always{}; }
        void return_void() { ready_.set_value(true); }
        void unhandled_exception() { ready_.set_exception(std::current_exception()); }
    };

    void get()
    {
        std::future<bool> result = coro_.promise().ready_.get_future();
